...
export default async function Page() {
    ...
    const { data: Contract } = await supabase.from('contracts_temp').select().order('date', { ascending: false })
    return (
        <div className="bg-black">
            <div className="flex items-center bg-black justify-between border-b border-white px-4 py-4 sm:px-6 sm:py-6 lg:px-8">
              <h1 className="text-base font-semibold leading-7 text-white">Dashboard | Alle utkast kontrakter</h1>
              <div className='flex items-center gap-x-2'>
              <a href="/dashboard/admin/contractform" className="bg-gray-600 rounded px-1 py-1 text-sm font-medium leading-5 text-white hover:text-indigo-100 hover:bg-gray-700">
                Lag et utkast
              </a>
              </div>
            </div>
            ...
            <ul role="list" className="divide-y divide-white/5">
              {Contract?.map((contracts: Contract, index) => (
                ...
                <a href={`/dashboard/admin/draft/${contracts.id}`}>
                <div
