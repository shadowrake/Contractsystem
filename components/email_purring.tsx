...
const PurringFrom: React.FC<Readonly<ContractFormInputs>> = ({id, input, email, contact, seller, buyer, orgnr, address, phone, total, comment, user_email, user_phone}) => {
    const monthPrice = total / 12;
    const intMonthPrice = Math.round(monthPrice);
  return <div>
      <h1>Påminnelse på Kontrakt fra Herman kristiansen</h1>
      ...
      <p>For å godkjenne/avvise kontrakten, klikk på lenken under</p>
      <a style={{
    ...
    }} href={`https://hermankristiansen.no/contractconfirm/underReview/${id}`}>
        Gå til godkjening
      </a>
